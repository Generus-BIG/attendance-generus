drop function if exists public.get_public_lupg_program_analytics_payload(text);
drop function if exists public.get_public_lupg_program_analytics_photo_paths(text);

create function public.get_public_lupg_program_analytics_payload(
  p_token text,
    ...
    select jsonb_build_object(
      ...
      'data', jsonb_build_object(
        ...
        'metricValues', coalesce((
          ...
          where report.month in (selected_month, previous_month)
        ), '[]'::jsonb),
        ...
        'sarprasValues', coalesce((
          ...
          where report.month = selected_month
        ), '[]'::jsonb),
...
revoke all on function public.get_public_lupg_program_analytics_payload(text, text)
  from public, anon, authenticated;
grant execute on function public.get_public_lupg_program_analytics_payload(text, text) to anon;

create function public.get_public_lupg_program_analytics_photo_paths(
  p_token text,
  p_month text default null
)
returns jsonb
...
begin
  ...
  return coalesce((
    ...
    submitted_reports as (
      ...
      from public.lupg_monthly_reports mr
      join groups g on g.id = mr.kelompok_id
      where mr.month = selected_month and mr.status = 'submitted'
    )
    select jsonb_agg(jsonb_build_object(
      'kelompokKey', report.group_key,
      'monthKey', to_char(report.month, 'YYYY-MM'),
...
revoke all on function public.get_public_lupg_program_analytics_photo_paths(text, text)
  from public, anon, authenticated;
grant execute on function public.get_public_lupg_program_analytics_photo_paths(text, text) to service_role;
