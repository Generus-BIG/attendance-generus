...
import {
  buildSlides,
  type Kelompok,
  type PresentationMonthlyReport,
} from './slides'
...
function PresentationLoader({
  ...
  const reportAuditQ = useQuery({
    queryKey: ['lupg', 'presentation-report-audit', reportIdsKey],
    queryFn: async () => {
      return Promise.all(
        reports.map(async (report) => {
          ...
          } satisfies PresentationMonthlyReport
        })
      )
    },
    enabled: !reportsLoading,
  })
  const presentationReports = reportAuditQ.data ?? reports
    ...
    queryFn: async (): Promise<ActivityPhotoWithUrl[]> => {
      ...
      return rows.map(
        (r: {
          ...
          kelompokName: kelompokFilter
            ? undefined
            : (() => {
                const report = reports.find(
                  (report) => report.id === r.report_id
                )
                return kelompokList.find(
                  (kelompok) => kelompok.id === report?.kelompok_id
                )?.value
              })(),
        })
      )
