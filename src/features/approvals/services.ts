...
import { buildMergeBirthDataUpdate } from './approval-utils'
...
export const approvalService = {
  async getPending(): Promise<PendingParticipant[]> {
    ...
    return data.map((d) => ({
      ...
      birthDate: d.birth_date
        ? parse(d.birth_date, 'yyyy-MM-dd', new Date())
        : null,
  ...
  async approve(
    ...
    if (isNew) {
      ...
      if (!groupId) {
        throw new Error(
          `Kelompok tidak ditemukan: ${pending.suggestedKelompok}`
        )
      }
      if (!categoryId) {
        throw new Error(
          `Kategori tidak ditemukan: ${pending.suggestedKategori}`
        )
      }
      ...
      const { data: newParticipant, error: createError } = await supabase
        ...
        .insert({
          ...
          birth_date: pending.birthDate
            ? format(pending.birthDate, 'yyyy-MM-dd')
            : null,
    ...
    if (!isNew) {
      const { data: existingParticipant, error: readTargetError } =
        await supabase
          .from('participants')
      ...
      if (readTargetError) throw readTargetError

      const birthDataUpdate = buildMergeBirthDataUpdate(
        pending,
        existingParticipant
      )
      if (Object.keys(birthDataUpdate).length > 0) {
        const { error: updateTargetError } = await supabase
          .from('participants')
          .update(birthDataUpdate)
          .eq('id', finalParticipantId)
